...
async def test_async_contest_list():
  ...
  assert c.authors[0] == CodeforcesUser(
      name="Aris",
...
async def test_async_contest_list2():
  ...
  assert c.authors == [
      ...
      CodeforcesUser(
          name="glebustim",
          title="Grandmaster glebustim",
          class__="user-red",
          profile="/profile/glebustim",
      ),
  ]
  assert c.start == 1695047700
  assert c.length == "02:00"
  assert c.participants == "17066"
  assert c.upcoming is True
  assert c.registered is False
  assert c.Div == ["D", "1", "2"]


def test_contest_list_page_18():
  from codeforces_core.contest_list import parse_contest_list_page, CodeforcesUser

  path = os.path.join(test_dir, "unit/mock", "contest_list_18.html")
  with open(path) as f:
    html = f.read()
    res = parse_contest_list_page(html)
    for r in res.history:
      if r.id == 79:
        assert r.authors[1] == CodeforcesUser(
            name="rng_58",
            title="Legendary Grandmaster rng_58",
            class__="user-legendary",
            profile="/profile/rng_58",
        )
