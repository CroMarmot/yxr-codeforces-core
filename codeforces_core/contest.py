...
def get_contest_info(cid):
  if cid:
    cur = db.cursor()
    return cur.execute(
        f'''SELECT title, authors, start, length, participants, upcoming FROM codeforces WHERE cid = {cid};'''
    ).fetchone()
  else:
    return []
...
def parse_contest_list(raw_contests, upcoming=0, write_db=True):
  ...
  for c in raw_contests.xpath('.//tr[@data-contestid]'):
    ...
    if write_db:
      cur.execute(
          'INSERT or REPLACE INTO codeforces (cid, title, authors, start, length, participants, registration, upcoming) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
...
def show_contests(contests, show_all=False, upcoming=False, solved_json=None):
  ...
  for c in contests:
    ...
    if solved_json and str(cid) in solved_json['solvedProblemCountsByContestId'] and str(
        cid) in solved_json['problemCountsByContestId']:
    ...
    if not show_all and conf['only_goals'] and (days > 0 or len(div) == 0 or conf['contest_goals'][div[-1]] == 0):
      puts = lambda msg: None
    contest_info = "{:04d} {:<3} {}{:<{width}} {} ({}){}{}{}".format(cid,
                                                                     div,
                                                                     solved_str,
                                                                     title[:conf['title_width']],
                                                                     start.strftime("%Y-%m-%d %H:%M"),
                                                                     length,
                                                                     weekday,
                                                                     countdown,
                                                                     participants,
                                                                     width=conf['title_width'])
    puts(contest_info)
  if not upcoming:
...
#    last_modified = int(cur.execute('''SELECT strftime('%s', last_modified) FROM modifications WHERE site = 'codeforces';''').fetchone()[0])
  ...
  if upcoming:
    ...
    upcoming = cur.execute(
        '''SELECT cid, title, authors, start, length, participants, registration FROM codeforces WHERE upcoming = 1 ORDER BY start;'''
    )
    show_contests(upcoming, show_all=args.all, upcoming=True)
  else:
    print("[+] Past contests")
    contests = cur.execute(
        '''SELECT cid, title, authors, start, length, participants, registration FROM codeforces WHERE upcoming = 0 ORDER BY start;'''
    )
...
async def async_register(args):
  ...
  try:
    ...
    form = _http.create_form(register_form)
